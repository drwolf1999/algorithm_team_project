...
int main() {
    ...
    DoYeop * dy = new DoYeop(REFERENCE_FILE.c_str(), SHORT_READ_FILE.c_str(), MISMATCH, 20);
    ...
    int cnt = 0;
    for (unsigned long i = 0; i < min(myDNA.size(), ans.size()); i++) {
        if (myDNA[i] == ans[i]) cnt++;
    }
    comp << double(cnt) * 100.0 / min(myDNA.size(), ans.size());
    return 0;
}